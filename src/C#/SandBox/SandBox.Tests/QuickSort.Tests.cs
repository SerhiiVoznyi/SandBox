...
public class QuickSortTests
{
    ...
    public void SortWithResult_Should_Sort_And_Return_Timing_Metadata()
    {
        ...
        var result = QuickSort.SortWithResult(values);
        ...
        Assert.Equal("O(n²)", result.TheoreticalWorstCaseBigO);
    ...
    {
        var empty = QuickSort.SortWithResult(Array.Empty<int>());
        var single = QuickSort.SortWithResult(new[] { 7 });
    ...
    [Fact]
    public void SortWithResult_Should_Observe_Quadratic_On_Already_Sorted_Large_Input()
    {
        // Last-element pivot + Lomuto degrades toward O(n²) on sorted data.
        var values = Enumerable.Range(1, 512).ToArray();

        var result = QuickSort.SortWithResult(values);

        Assert.Equal(Enumerable.Range(1, 512), values);
        Assert.Equal("O(n²)", result.ObservedBigO);
